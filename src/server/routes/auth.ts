...
import { generateSessionId, hashToken, verifyPassword } from "../../lib/crypto";
import { logger } from "../../lib/logger";
import { resolveSecretCached } from "../../lib/secrets";
import type {
  LocalUserSpec,
  LocalUserStatus,
} from "../../types/crd/local-user";
...
export async function registerAuthRoutes(
  ...
  app.post("/api/v1/auth/login", async (request, reply) => {
    ...
    try {
      ...
      const userSpec: LocalUserSpec = JSON.parse(matchedUser.spec || "{}");
      ...
      if (
        ...
        "passwordHash" in userSpec &&
        userSpec.passwordHash
      ) {
        passwordHash = userSpec.passwordHash as string;
        logger.debug(
          { username },
          "Using passwordHash from spec (dev mode only)",
        );
