...
import type { JobManager, JobManagerConfig, JobResult, Monitor, JobStatus } from "./types";
...
export function createJobManager(config: JobManagerConfig): JobManager {
	...
	async function scheduleCheck(monitor: Monitor): Promise<JobResult> {
		...
		try {
			...
			logger.debug({ namespace, jobName: job.metadata?.name || "unknown" }, "Creating Job with namespace");
...
export type { JobManager, JobManagerConfig, JobResult, Monitor, JobStatus } from "./types";
